...
# Параметры Qwen2-VL-7B (на основе фактических весов)
VIS_DIM = 5120  # Входная размерность visual encoder
LLM_DIM = 3584  # Выходная размерность (размерность Qwen2.5-7B LLM)
N_BOXES = 5

projector = VisualToTextEmbeddingProjector(
    visual_dim=VIS_DIM,
    text_dim=LLM_DIM,
    target_dim=LLM_DIM  # MLP: 5120 -> 5120 -> 3584
).to(device).to(torch.bfloat16)  # Используем BFloat16 как в оригинальной модели

# Имитация входа из энкодера (в BFloat16)
batch_global = torch.randn(2, VIS_DIM, dtype=torch.bfloat16).to(device)
batch_boxes = torch.randn(2, N_BOXES, VIS_DIM, dtype=torch.bfloat16).to(device)
batch_text = torch.randn(2, LLM_DIM, dtype=torch.bfloat16).to(device)
from load_qwen_weights import load_vision_projector_weights

# Загрузка весов vision projector с Hugging Face
